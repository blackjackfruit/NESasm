...
char *outputPathWithFileName;   /* path to output file to */
		...
		for (i = 1; i < argc; i++) {
			if (argv[i][0] == '-') {
                ...
                /* input file(s) */
                    ...
                    int nextParameterIndex = i+1;
                    if (nextParameterIndex < argc) {
                        strcpy(in_fname, argv[nextParameterIndex]);
                        file++;
                    } else {
                ...
                /* output path */
                    ...
                    int nextParameterIndex = i+1;
                    if (nextParameterIndex < argc) {
                        char *pathForOutput = argv[nextParameterIndex];
                        outputPathWithFileName = malloc(strlen(pathForOutput));
                        if (outputPathWithFileName == NULL) {
                            releaseMallocs();
                            printf("Failed to allocate memory");
                            return 1;
                        }
                        strcpy(outputPathWithFileName, argv[nextParameterIndex]);
                    }else {
                        printf("Output switch set but not path was added");
                        help();
                        return 1;
                    }
                }
	...
	if (errcnt == 0) {
		...
		/* save */
			...
			/* binary file */
				...
				/* open file */
				if ((fp = fopen(outputPathWithFileName, "wb")) == NULL) {
                    releaseMallocs();
